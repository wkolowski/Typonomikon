...
Proof.
  split; intro.
  - apply Prefix_rev_l in H.
    now rewrite Palindrome_spec.
  - apply Palindrome_spec in H.
    rewrite <- H.
    now apply Prefix_refl.
Qed.
(* end hide *)
...
Proof.
  split; intros H.
  - apply Subseq_rev in H.
    now rewrite 2!rev_rev in H.
  - now apply Subseq_rev.
Qed.
(* end hide *)
...
Proof.
  intros.
  apply Subseq_rev in H.
  rewrite 2!rev_snoc in H.
  apply Subseq_cons_inv in H.
  apply Subseq_rev in H.
  now rewrite 2!rev_rev in H.
Qed.
(* end hide *)
...
Proof.
  split; cycle 1.
  - intros <-%Palindrome_spec.
    now apply Subseq_refl.
  - induction l using list_palindrome_ind; cbn; [now constructor.. |].
    rewrite rev_snoc; cbn.
    inversion 1; subst.
    + constructor.
      apply IHl.
      now apply Subseq_snoc_inv in H1.
    +
Abort.
(* end hide *)
...
Proof.
  induction m as [| m']; cbn; intros.
    ...
    destruct (list_eq_dec_spec eq_dec_spec l1 (drop (S m') l2 ++ take (S m') l2)); cbn.
