...
Require Import Recdef.
Function cycle {A : Type} (n : nat) (l : list A) : list A :=
match n, l with
    | 0, _ => l
    | S n', [] => []
    | S n', h :: t => cycle n' (t ++ [h])
...
Compute cycle 7 [1; 2; 3; 4; 5; 6; 7].
Compute cycle' 11 [1; 2; 3; 4; 5; 6; 7].
...
Proof.
  induction n as [| n']; cbn.
    ...
    destruct l as [| h t]; cbn.
      reflexivity.
      rewrite IHn', length_app, plus_comm. cbn. reflexivity.
Qed.
  ...
  induction m as [| m']; cbn.
    ...
    destruct n as [| n']; cbn.
      ...
      intro. cbn in IHm'. rewrite <- IHm'. f_equal.
        rewrite <- snoc_app_singl, snoc_replicate. cbn. reflexivity.
Qed.

Lemma cycle_zipWith :
  forall
    (A B C : Type) (f : A -> B -> C)
    (n : nat) (la : list A) (lb : list B),
      cycle n (zipWith f la lb) =
      zipWith f (cycle n la) (cycle n lb).
Proof.
  induction n as [| n']; cbn.
    reflexivity.
    destruct la as [| ha ta]; cbn.
      reflexivity.
...
Lemma cycle_length :
  forall (A : Type) (n : nat) (l : list A),
    cycle (n + length l) l = cycle n l.
Proof.
  induction n as [| n']; cbn; intros.
    admit.
    destruct l as [| h t]; cbn.
      reflexivity.
      rewrite <- plus_n_Sm. cbn. destruct t as [| ht tt]; cbn.
...
Proof.
  induction l1 as [| h t]; cbn; intros.
    ...
    rewrite <- app_assoc, IHt, <- app_assoc. cbn. reflexivity.
Qed.
...
Compute cycle 5 [1; 2; 3; 4; 5; 6; 7].
Compute rev (cycle 5 (rev [1; 2; 3; 4; 5; 6; 7])).
Compute cycle 5 (rev [1; 2; 3; 4; 5; 6; 7]).
Compute rev (cycle 5 [1; 2; 3; 4; 5; 6; 7]).

Lemma cycle_rev :
  forall (A : Type) (n : nat) (l : list A),
    cycle n l = rev (cycle n (rev l)).
Proof.
  induction n as [| n']; cbn; intros.
    rewrite rev_inv. reflexivity.
    destruct l as [| h t]; cbn.
      reflexivity.
      rewrite IHn'. f_equal. rewrite rev_app. cbn.
        case_eq (rev t ++ [h]); intros.
          admit.
Abort.
...
Proof.
  induction n as [| n']; cbn.
    ...
    destruct l as [| h t]; cbn.
      reflexivity.
      rewrite <- IHn', map_app. cbn. reflexivity.
Qed.
...
Proof.
  induction n as [| n']; cbn.
    ...
    destruct l as [| h t]; cbn.
      reflexivity.
      rewrite IHn', all_app, andb_comm. cbn.
...
Proof.
  intros. rewrite !any_all.
  f_equal. rewrite all_cycle.
  reflexivity.
Qed.
...
Proof.
  induction n as [| n']; cbn.
    ...
    destruct l as [| h t]; cbn.
      reflexivity.
      rewrite IHn', filter_app. cbn. destruct (p h).
...
Proof.
  induction n as [| n']; cbn.
    ...
    destruct l as [| h t]; cbn.
      reflexivity.
      rewrite IHn', count_app. cbn. destruct (p h).
...
Proof.
  induction n as [| n']; cbn.
    ...
    destruct l as [| h t]; cbn.
      ...
      eapply Cycle_trans.
        apply IHn'.
        constructor. rewrite snoc_app_singl. apply Cycle_refl.
Qed.
...
Proof.
  induction n as [| n']; cbn.
    ...
    destruct l as [| h t]; cbn.
      reflexivity.
      rewrite IHn'. rewrite Permutation_app_comm. cbn. reflexivity.
Restart.
  intros. apply Cycle_Permutation. apply Cycle_cycle.
Qed.
...
Proof.
  induction 1.
    ...
    destruct IHCycle as [n IH]. exists (n + length l2).
      ...
      rewrite IH, snoc_app_singl, (plus_n_O (length l2)).
      rewrite cycle_length_app. cbn. reflexivity.
Qed.
...
Proof.
  induction n as [| n']; cbn.
    ...
    destruct l as [| h t]; cbn.
      reflexivity.
      rewrite IHn', Exists_app, !Exists_cons, !Exists_nil.
        firstorder.
Qed.
