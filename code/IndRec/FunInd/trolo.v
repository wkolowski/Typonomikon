...
(*
Fixpoint cbn (t : Term Var) (env : Env) : nat :=
match t with
    | C n => n
    | V x => lookup x env
    | Add t1 t2 => plus (cbn t1) (cbn t2)
    | App t1 x t2 => cbnh t1 x t2
end

with cbnh (t : Term Var) (y : Var) (z : Term Var) : Term Var :=
match t with
    | C n => C n
    | V x => if veq x y then cbnh z else V x
    | Add t1 t2 => Add (cbn t1 y z) (cbn t2 y z)
    | App t1 x t2 => cbn (cbn t1 x t2) y z
end.
*)
...
Fixpoint cbn
  (t : Term Var) (y : Var) (z : Term Var) {struct t} : Term Var :=
match t with
    | C n => C n
    | V x => if veq x y then cbnh z else V x
    | Add t1 t2 => Add (cbn t1 y z) (cbn t2 y z)
...
with cbnh (t : Term Var) : Term Var :=
match t with
    | C n => C n
    | V x => V x
    | Add t1 t2 => Add (cbnh t1) (cbnh t2)
    | App t1 x t2 => cbn t1 x t2
end.
Set Guard Checking.

Lemma cbnG_cbn :
  forall (t : Term Var) (y : Var) (z : Term Var),
    cbnG t y z (cbn t y z)
...
Proof.
  destruct t; cbn; intros.
    constructor.
    destruct (veq_spec v y).
      subst. constructor. apply cbnhG_cbnh.
