...
Fixpoint toBT {A : Set} {n : nat} (et : EnhBTree A n)
  : BTree A :=
match et with
    | ENil => empty
...
Theorem trolo :
  forall (A B : Set) (n : nat) (f : A -> B) (et : EnhBTree A n),
    toBT (emap f et) = map f (toBT et).
Proof.
  induction et; auto. simpl. f_equal. apply IHet1. apply IHet2.
Qed.

Instance Functor_EnhBTree (n : nat)
  : Functor (fun A : Set => EnhBTree A n).
Proof.
  split with (@emap n);
...
Theorem any_height_inj2 :
  forall (n m : nat) (t1 : EnhBTree nat n)
