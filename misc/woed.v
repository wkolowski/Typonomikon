...
(** BEWARE: these notes are still incomplete. The subchapters already written
    shouldn't change, but new ones will appear and the overall order of
    subchapters may change. I will soon try to specify which exercises are to
    to be done to receive a particular mark.
...
(* ===> prop_extensionality = forall A B : Prop, A <-> B -> A = B
     : Prop *)
...
(** Because [(A : Type)] appears before the final semicolon, [option] is
    not a type, but a family of types, parametrized by a type [A]. We can
    see this with the [Check] command. *)
...
(** These exercises are stolen from the first chapter of Essentials of
    Programming Languages (exerciss 1.1, 1.2). Each one is in a separate
...
(** We have the same problem here. An inductive hypothesis is missing that
...
(** This is exactly what we need. First, we have two predicates, [P] and [P0].
    ...
    Coq. [P0] is the inductive hypothesis that was missing in our last attempt
...
(* ===> swap_blocks_ind :
       ...
       (forall (l : list A) (x : A) (l0 : list A),
        ...
        forall (y : A) (t : list A),
        l0 = y :: t ->
        P t (swap_blocks t) -> P (x :: y :: t) (y :: x :: swap_blocks t)) ->
...
(** We are now ready to learn about custom induction principles. But it will
    be easier to understand that topic (especially the inner workings of the
    commands [Function] and [Functional Scheme]), if we first see how we can
...
(** The difference is quite stark: the recursive one ([div2]) tells us how
    ...
    a procedure dividing its argument by two. Nonetheless these two
...
Theorem div2_rel_div2 :
  forall n m : nat, div2 n = m <-> div2_rel n m.
(* begin hide *)
Proof.
  split.
    generalize dependent m. induction n using nat_ind_2; cbn; intros; subst.
...
(** This theorem says that [div2_rel] is the graph of [div2]. Therefore
    [div2_rel] may also be thought of as a specification for [div2]. But
    specifying functions is not everything we can achieve using inductive
    relations. Another thing is specifying partial functions. *)
...
Inductive Zfact : Z -> Z -> Prop :=
    | Zfact_0 : Zfact 0 1
    | Zfact_S : forall (k r : Z), Zfact k r -> Zfact (k + 1) ((k + 1) * r).
...
(** The point is that [R_div2'_ind], the induction principle for the graph
    ...
    after all, generated automagically), but nonetheless it's a fact. There's
    something very deep going on here.
    ...
    exactly f recursion's shape). Think about this until you understand it.

    Now that you have understood this illuminating fact, let's see how to
    derive the functional induction principle for [div2] by hand. *)
...
(** The first thing the commands [Function] and [Functional Scheme] do is
    defining the graph of the function in question. [R_div2] is the graph
...
(** Finally we can prove the induction principle for [div2] by using the
    induction principle for [R_div2']. *)
...
(** In general to prove [div2_ind'] we could need [div2_eq], which is
...
(** * Generalizing the inductive hypothesis (TODO) *)

(** * Technical shortcomings of [induction] (TODO) *)